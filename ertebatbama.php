...
<head>
  ...
  <link rel="stylesheet" href="fontawesome-free-5.15.1-web/css/all.css">
    <title>ارتباط با ما | رایان آموز</title>
</head>
<body dir="rtl">
...
<div style="margin-top: 0px;" class="div-main-aside" >
<main>
...
<div class="div-form" >
<p id="a" > لطفا موارد خواسته شده را وارد کنید !</p>
    <form id="form" action="sabtnazar.php"  method="POST">
       <p class="p-comment" > نظرات و پیشنهادات خود را با ما به اشتراک بگذارید </p>  
        <input id="a1"  class="input1" type="text" name="name"  maxlength="30" minlength="3" placeholder="*نام خودرا وارد نمایید" required>
        <input id="a2" class="input1" type="text" name="subject" minlength="3" maxlength="50" placeholder="*عنوان"  required>
        <textarea id="a3" minlength="3" name="text"  cols="80" rows="10" placeholder="* نظر شما . . ."  required></textarea>
        <button onclick="sub()" name="submit" type="submit" class="input-btn btn1" >ثبت </button>
        <button  class="input-btn btn1" type="reset"> انصراف</button> 
    </form>
    <br>
    <div>
...
if(mysqli_connect_errno()){    echo "خطا در اتصال".mysqli_connect_error();}
$r=mysqli_query($con,"SELECT * FROM `ertebat1`");
while($row=mysqli_fetch_array($r)){
    
    echo "<div class=div-table> <table> <tr> <td class=td1 id=t1 ><img class=img-user src=pic/user.png ></td><td class=td1  style=width:130px; >  " .$row['name']."  </td><td class=td1> عنوان: ". $row['subject']." </td> </tr><tr> <td colspan=3 class=td2> ".$row['text']." </td></tr></table>  "." </div>"   ;
}
mysqli_close($con);
?>
...
function sub(){
 if(document.getElementById('a1').value=='' || document.getElementById('a2').value=='' || document.getElementById('a3').value=='' )   {
